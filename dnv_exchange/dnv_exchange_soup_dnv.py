...
def main(requested_runs=40, wishlist_filename='wishlist.txt', **kwargs):
    ...
    wishlist_n = wishlist_filename.split('.')[0]
    db_output_filename = kwargs.pop('db_output_filename', '%s_output.txt' % wishlist_n)
    ids_completed_filename = kwargs.pop('ids_completed_filename', '%s_processed.txt' % wishlist_n)
    log_filename = kwargs.pop('log_filename', '%s_log.txt' % wishlist_n)
    ...
    with open(wishlist_filename, 'r') as f:
        ...
        if requested_runs > len(dnv_wish_list):
            ...
            print '%s is empty.\nProvide a list of dnv_id values to search.' % dnv_wish_list
        else:
    ...
    for dnv_id in part_list[:]:
        with open(log_filename, 'a') as f:
            message = "%s of %s : Processing dnv_id = %s" % \
                      (cycle_count, number_of_requests, str(dnv_id))
            f.write(message + '; ')
            if verbose:
                print message
...
main(requested_runs=400,
     ...
     log_filename='./dnv_exchange_gets/dnv_db_get_log.txt',
     db_output_filename='./dnv_exchange_gets/dnv_db_get.txt')
'''
